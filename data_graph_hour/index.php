...
while ($row = $stations->fetchArray(1)) {
    ...
    $statement = $db->prepare("SELECT temperature, ambient_light, barometric_pressure, time FROM Readings where node_id = :id and time >= datetime('now', 'localtime', '-1 hour') order by time desc");
     //assign :id
