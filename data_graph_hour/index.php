...
while ($row = $stations->fetchArray(1)) {
    ...
    $statement = $db->prepare("SELECT temperature, ambient_light, barometric_pressure, 'time' FROM Readings where node_id = :id and time >= date('now', '-1 hours') and 'time'< date('now') order by time desc");
     //assign :id
